        ...
        <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
          <div className="flex items-center gap-3 relative">
            <div className="relative w-fit">
              ...
              {/* O BOTÃO 'NOVO LEAD' FOI REMOVIDO DAQUI E MOVIMENTADO PARA O FAB */}
              <div className="flex gap-3 flex-wrap">
                {resumoMap.map(({ key, label, color, icon: Icon }) => (
                  (resumo[key] > 0 || key === 'total') ? (
                ...
                <div className="overflow-x-auto rounded-2xl shadow-lg bg-white border border-gray-200">
                  <table className="min-w-full text-sm">
                    ...
                    <tbody>
                      {leads.filter(lead => { // Usar 'leads' diretamente aqui, já filtrado pelo App.js
                        const d = parseISODate(lead.dataCadastro);
                        const start = new Date(appliedRange[0].startDate);
                        const end = new Date(appliedRange[0].endDate);
                        ...
                        // Aplicar filtros da tabela aqui também para o array 'leads'
                        ...
                        // Filtro principal de "Pré-Vendas": DEVERIA EXCLUIR APENAS AGENDADOS.
                        // Para depuração, temporariamente não excluir nada aqui.
                        return dataOk && buscaOk && statusFiltroOk; // <--- MUDADO PARA VER TODOS OS LEADS
                      }).map((lead) => (
