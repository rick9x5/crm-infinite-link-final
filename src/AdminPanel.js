...
function parseISODate(str) {
  ...
  if (!str || typeof str !== 'string' || !str.match(/^\d{4}-\d{2}-\d{2}$/)) {
    // Se a string for inválida ou undefined, retorna a data de HOJE
        ...
        <header className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
          ...
          <div className="flex gap-3 flex-wrap">
            {resumoMap.map(({ key, label, color, icon: Icon }) => (
              // Mostra todos os status do resumo, mesmo se a contagem for zero, para que o usuário possa clicar.
              // A classe condicional garante que o status 'total' esteja sempre visível e selecionável
...
                  <div><label className="text-sm">Email</label><input name="email" value={newLeadData.email} onChange={handleNewLeadChange} className={inputStyle} /></div>
